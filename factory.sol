...
contract TusswapPair is TusswapERC20 {
    using SafeMathTusswap  for uint;
    ...
    modifier lock() {
        require(unlocked == 1, 'Tusswap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Tusswap: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'Tusswap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        uint balance0 = IERC20Tusswap(_token0).balanceOf(address(this));
        uint balance1 = IERC20Tusswap(_token1).balanceOf(address(this));
        ...
        balance0 = IERC20Tusswap(_token0).balanceOf(address(this));
        balance1 = IERC20Tusswap(_token1).balanceOf(address(this));
    ...
    function swap(uint amount0Out, uint amount1Out, address to) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'Tusswap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'Tusswap: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'Tusswap: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'Tusswap: INSUFFICIENT_INPUT_AMOUNT');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'Tusswap: K');
        }
    ...
    function sync() external lock {
        _update(IERC20Tusswap(token0).balanceOf(address(this)), IERC20Tusswap(token1).balanceOf(address(this)), reserve0, reserve1);
    }
...
contract TusswapFactory is ITusswapFactory {
    ...
    event PairAdded(address indexed token0, address indexed token1, address pair, uint);
    ...
    function addPair(address tokenA, address tokenB,address pair) external {
        ...
        getPair[token1][token0] = pair; // populate mapping in the reverse direction
        allPairs.push(pair);
        emit PairAdded(token0, token1, pair, allPairs.length);
    }
